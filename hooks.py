...
def test_hook_default(
    ...
    get_key_metric2=itemgetter('accuracy')
):
    ...
    for i, sample in enumerate(iterator):
        ...
        aji=metric_fn.give_value(true=targets, pred=predict)
        
        f1=metric_fn2.f1(y_true=targets,y_pred=predict)
        

        AJI.append(get_key_metric(aji))
        
        F1.append(get_key_metric2(f1))
        

        final_AJI=np.mean(AJI)
        ...
        if get_key_metric is not None:
            status = status + " AJI: {0:.5} ".format(final_AJI)
            status= status+ ", F1: {0:.5}".format(final_F1)
        iterator.set_description(status)
    output = {"metric": {'AJI':final_AJI}, "loss": loss_avg.avg, 'F1':{"F1": final_F1}}
    return output
...
def end_epoch_hook_semseg(iterator, epoch, output_train, output_test):
    ...
    if hasattr(iterator, "set_description"):
        iterator.set_description(
            "epoch: {0}, AJI: {1:.5}, F1: {1:.5}, train_loss: {2:.5}, test_loss: {3:.5}".format(
                epoch, output_test["metric"]["AJI"], output_test["F1"]["F1"], output_train["loss"], output_test["loss"]
            )
        )
