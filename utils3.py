...
def patches(data_path,imgs_folder,masks_folder,im_width,im_height,Aug=False, patch_width=256,patch_height=256):
    ...
    y_train=[]
    for name in names:
        if  Aug: #for reading augmentation files only.
            name=name.split('.')[0]
            ID=name.split('_')[0]
            Num=name.split('_')[2]
            #print(name+'_bin_mask.png')
            image= os.path.join(data_path, imgs_folder, ID +'_aug_' + Num +  '.png')
            mask= os.path.join(data_path, masks_folder, ID+'_aug_' + 'mask_'+ Num + '.png')
            
        else:
            name=name.split('.')[0]
            image=os.path.join(data_path,imgs_folder, name + '.png')
            mask=os.path.join(data_path, masks_folder, name +'_bin_mask.png')
        
        x_img = cv2.imread(image,1)
        #x_img = x_img/255.0
        
        # Load y masks
        mask = np.expand_dims(cv2.imread(mask,0),axis=2)
        #mask=mask/255.0
...
def patches_test(data_path,imgs_folder,masks_folder,im_width,im_height,patch_width=256,patch_height=256):
    ...
    for name in names:
        ...
        #X_train[count] = x_img/255.0
        #y_train[count] = mask/255.0
        new_imgs = view_as_windows(x_img, (patch_width, patch_height, 3), (patch_width, patch_height, 3))
        #print("Number of Patches")
        #print(new_imgs.shape)
        for patch in new_imgs:
            X_train.append(patch)
        new_masks = view_as_windows(mask, (patch_width, patch_height, 1), (patch_width, patch_height, 1))
        for patch in new_masks:
            y_train.append(patch)
